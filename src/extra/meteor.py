...
def retrieve_data(docker_manager, config):
    ...
    for file in config['extra_files']:
        ...
        wall_clock_pattern = (
            r"Wall Clock time for Test Meteor RELU: ([\d.]+) sec"
        )
        cpu_time_pattern = r"CPU time for Test Meteor RELU: ([\d.]+) sec"
        total_comm_pattern = (
        ...
        party_comm_pattern = (
            r"Communication, Test Meteor RELU, (P\d+): ([\d.]+)MB \(sent\) "
            r"([\d.]+)MB \(recv\)"
        )
        party_rounds_pattern = (
            r"Rounds, Test Meteor RELU, (P\d+): (\d+)\(sends\) (\d+)\(recvs\)"
        )
